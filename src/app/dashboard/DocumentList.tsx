  ...
  if (documents.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-gray-300 p-6 text-center text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">
        No documents yet. Upload a supplier PDF or Shopify CSV to get started.
      </p>
    );
  }
          ...
          >
            <div className="flex flex-col">
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400">
                {csv ? "Shopify CSV" : "Supplier PDF"} ·{" "}
                {new Date(doc.uploaded_at).toLocaleString()}
                {doc.page_count
                  ? ` · ${doc.page_count} ${csv ? "rows" : "pages"}`
                  : ""}
              </span>
